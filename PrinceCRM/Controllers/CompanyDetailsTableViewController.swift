...
class CompanyDetailsTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        if let c = company {
            print(c)
            print("\(c.id) \(c.companyname) \(c.address) \(c.tel) \(c.fax) \(c.website) \(c.employeesnumber)")
        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 1 || indexPath.row == 2 {
            ...
            cell?.longLabel.text = company?.getStringFromIndex(indexPath.row).0
            cell?.longTextView.text = company?.getStringFromIndex(indexPath.row).1
            
            return cell as! UITableViewCell
        } else {
            let cell = tableView.dequeueReusableCell(withIdentifier: "cellShort", for: indexPath) as? CompanyDetailsTableViewCell
            cell?.shortLabel.text = company?.getStringFromIndex(indexPath.row).0
            cell?.shortTextField.text = company?.getStringFromIndex(indexPath.row).1
            return cell as! UITableViewCell
        }
